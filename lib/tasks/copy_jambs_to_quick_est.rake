namespace :sam_quick_est do
  desc "Copy jamb items from pricebook to Sam Quick Est"
  task copy_jambs: :environment do
    puts "Searching for items with 'jamb' in item_name..."

    jamb_items = PricebookItem.active.where('LOWER(item_name) LIKE ?', '%jamb%')

    puts "Found #{jamb_items.count} jamb items in pricebook"

    if jamb_items.count == 0
      puts "No jamb items found. Exiting."
      exit 0
    end
    ...
    jamb_items.find_each.with_index do |item, index|
      begin
