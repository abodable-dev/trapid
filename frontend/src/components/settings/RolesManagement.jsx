...
export default function RolesManagement() {
  ...
  const loadRoles = async () => {
    try {
      setLoading(true)
      const response = await api.get('/api/v1/user_roles')
  ...
  const handleAddRole = async () => {
    if (!newRoleName.trim() || !newRoleLabel.trim()) {
      setToast({
        message: 'Both role name and label are required',
        type: 'error'
      })
      return
    }
    ...
    try {
      await api.post('/api/v1/user_roles', {
      ...
      setToast({
        message: err.response?.data?.error || 'Failed to add role',
        type: 'error'
      })
    }
  }
  ...
  const handleDeleteRole = async (roleName) => {
    if (!confirm(`Are you sure you want to delete the role "${roleName}"? Users with this role will need to be reassigned.`)) {
      return
    }

    try {
      await api.delete(`/api/v1/user_roles/${roleName}`)
      setToast({
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="sm:flex sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
            Define system permission levels. Roles control what users can do in the application.
          </p>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
        <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-4">Add New Role</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label htmlFor="roleName" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
              Role Name (lowercase, underscore-separated)
            </label>
            <input
              id="roleName"
              type="text"
          ...
          <div>
            <label htmlFor="roleLabel" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
              Display Label
            </label>
            <input
              id="roleLabel"
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
        <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
          <h3 className="text-lg font-medium text-gray-900 dark:text-white">Current Roles</h3>
        </div>
        <div className="divide-y divide-gray-200 dark:divide-gray-700">
          {roles.length === 0 ? (
            <div className="px-6 py-12 text-center">
              <ShieldCheckIcon className="mx-auto h-12 w-12 text-gray-400" />
              <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">No roles defined</p>
            </div>
          ) : (
            roles.map((role) => (
              <div
      ...
      <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
        <div className="flex">
          ...
          <div className="ml-3">
            <h3 className="text-sm font-medium text-blue-800 dark:text-blue-200">About Roles</h3>
            <div className="mt-2 text-sm text-blue-700 dark:text-blue-300">
              <ul className="list-disc list-inside space-y-1">
                <li>Roles define system permission levels (e.g., admin, user, supervisor)</li>
                <li>Each user must have exactly one role</li>
                <li>Role names should be lowercase with underscores (e.g., project_manager)</li>
