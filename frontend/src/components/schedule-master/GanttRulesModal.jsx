...
/**
 * GanttRulesModal - Displays the GANTT_SCHEDULE_RULES.md documentation
 */
export default function GanttRulesModal({ isOpen, onClose }) {
  ...
  const fetchRules = async () => {
    try {
      setLoading(true)
      // Fetch the markdown file from the project root
      const response = await fetch('/GANTT_SCHEDULE_RULES.md')
      const text = await response.text()
      setContent(text)
    } catch (err) {
      console.error('Failed to load Gantt rules:', err)
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-xl font-semibold text-gray-900 dark:text-white">
                Gantt & Schedule Master - Development Rules
              </h2>
            </div>
            <button
              onClick={onClose}
