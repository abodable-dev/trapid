...
export default function TaskTable({ tasks = [], onTaskUpdate, colorConfig, colorBy = 'status', saving = false }) {
  ...
  const EditableDateCell = ({ task, field, value }) => {
    ...
    if (editing) {
      return (
        <input
          ...
          className="w-full px-2 py-1 text-sm border border-indigo-500 rounded focus:outline-none focus:ring-2 focus:ring-indigo-500 dark:bg-gray-800 dark:text-white disabled:opacity-50 disabled:cursor-not-allowed"
          disabled={saving}
          autoFocus
        />
      )
    ...
    return (
      <div
        ...
        className={`px-2 py-1 text-sm text-gray-900 dark:text-white rounded transition-colors ${saving ? 'cursor-wait opacity-60' : 'cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700'}`}
      >
        {formatDate(value)}
      </div>
    )
    ...
    return (
      <Listbox value={selected} onChange={handleChange}>
        <div className="relative">
          <ListboxButton className="relative w-full cursor-pointer rounded-md bg-white dark:bg-gray-800 py-1.5 pl-3 pr-8 text-left text-sm text-gray-900 dark:text-white hover:bg-gray-50 dark:hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 border border-transparent shadow-sm">
            {selected === null ? (
          ...
          <ListboxOptions className="absolute z-10 mt-1 max-h-60 w-full min-w-[200px] overflow-auto rounded-md bg-white dark:bg-gray-800 py-1 text-sm shadow-lg ring-1 ring-black ring-opacity-5 dark:ring-gray-700 focus:outline-none">
            <ListboxOption
              value={null}
  ...
  const SupplierListbox = ({ task }) => {
    ...
    return (
      <Listbox value={selected} onChange={handleChange}>
        <div className="relative">
          <ListboxButton className="relative w-full cursor-pointer rounded-md bg-white dark:bg-gray-800 py-1.5 pl-3 pr-10 text-left text-sm text-gray-900 dark:text-white hover:bg-gray-50 dark:hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-indigo-500">
            <span className="block truncate">{selected || 'No supplier'}</span>
            <span className="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-2">
              <ChevronUpDownIcon className="h-4 w-4 text-gray-400" aria-hidden="true" />
            </span>
          </ListboxButton>
          <ListboxOptions className="absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded-md bg-white dark:bg-gray-800 py-1 text-sm shadow-lg ring-1 ring-black ring-opacity-5 dark:ring-gray-700 focus:outline-none">
            <ListboxOption
              value={null}
              className={({ active }) =>
  ...
  return (
    <div className="bg-white dark:bg-gray-900 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden" style={{ height: 'calc(100vh - 280px)', minHeight: '400px', display: 'flex', flexDirection: 'column' }}>
            ...
            {sortedTasks.map((task, index) => (
              ...
              >
                ...
                <td className="px-3 py-3 whitespace-nowrap">
                  {colorBy === 'status' ? (
                    <span className={`inline-flex items-center px-2 py-1 rounded-md text-xs font-medium ${getBadgeStyle(task)}`}>
