...
export default function SamQuickEstPage() {
 ...
 const loadSamQuickEst = async (page = 1) => {
 try {
 if (page === 1) {
 setLoading(true)
 } else {
 ...
 console.error('Failed to load Sam Quick Est:', err)
 setItems([])
 setHasMore(false)
 } finally {
 setLoading(false)
 ...
 // Don't clear items - the useEffect will reload and pagination reset happens in loadSamQuickEst
 ...
 try {
 await api.patch(`/api/v1/sam_quick_est/${itemId}`, {
 sam_quick_est_item: {
 ...
 const url = `${API_URL}/api/v1/sam_quick_est/export?${params.toString()}`
 ...
 let filename = `sam_quick_est_${date}`
 if (selectedItems.size > 0) {
 ...
 <tr
 key={item.id}
 onClick={() => navigate(`/sam-quick-est/${item.id}`)}
