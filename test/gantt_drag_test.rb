...
# Usage: rails runner test/gantt_drag_test.rb

puts "🧪 GANTT DRAG TEST #1: Move Task 1 by 5 days"
puts "=" * 60

# Find template and tasks
template = ScheduleTemplate.find_by(id: 1)
unless template
  puts "❌ ERROR: Template #1 not found"
  exit 1
end
...
# Get tasks
task1 = template.schedule_template_rows.find_by(id: 299) # Task 1
task2 = template.schedule_template_rows.find_by(id: 300) # Task 2 (depends on Task 1)
task3 = template.schedule_template_rows.find_by(id: 301) # Task 3 (depends on Task 1)

unless task1 && task2 && task3
  puts "❌ ERROR: Tasks not found (299, 300, 301)"
  exit 1
end
