...
description: Use this agent when making product decisions, planning features, reviewing code for public readiness, or evaluating whether functionality should be internal-only vs. public-facing. This agent should be consulted proactively during:\n\n<example>\nContext: Developer is implementing a new feature for managing construction schedules\nuser: "I've added a new scheduling feature that integrates with Tekna's internal project management system"\nassistant: "Let me use the product-owner-analyst agent to evaluate whether this feature should be scoped for internal-only use or if it has potential for the public product."\n<commentary>\nSince a new feature is being added, use the Task tool to launch the product-owner-analyst agent to assess its public vs. internal classification.\n</commentary>\n</example>\n\n<example>\nContext: Developer is reviewing a pull request that adds technical assistance buttons\nuser: "I've completed the code review for the PR that adds some helper buttons for our support team"\nassistant: "Before we merge this, let me use the product-owner-analyst agent to ensure these technical assistance features are properly marked as internal-only and won't accidentally ship to external users."\n<commentary>\nSince code is being reviewed that may contain internal-specific features, use the product-owner-analyst agent to verify proper categorization.\n</commentary>\n</example>\n\n<example>\nContext: Planning session for upcoming sprint\nuser: "We need to prioritize the backlog for next sprint"\nassistant: "I'm going to use the product-owner-analyst agent to help categorize and prioritize features based on whether they're core product functionality or Tekna-specific internal tools."\n<commentary>\nUse the product-owner-analyst agent proactively during planning to ensure proper feature categorization.\n</commentary>\n</example>
model: opus
color: green
---
