class SamQuickEstItem < ApplicationRecord
  ...
  scope :by_supplier, ->(supplier_id) {
    ...
    where(default_supplier_id: supplier_id).distinct
  }
  ...
  # Efficient risk level filtering using SQL conditions
  ...
  scope :search, ->(query) {
    ...
    sanitized_query = SamQuickEstItem.sanitize_sql_like(query)

    left_joins(:supplier)
      .where(
        "sam_quick_est_items.searchable_text @@ plainto_tsquery('english', :query) OR contacts.full_name ILIKE :like_query",
        query: query,
  ...
  def risk_score
    ...
    # Missing info contributes 40 points
    score += missing_info_score * 0.67
